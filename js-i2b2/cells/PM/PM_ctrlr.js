 ...
 * @version 1.5
 *          ----------------------------------------------------------------------------------------
 *          updated 9-15-09: Refactor loading process to allow CELL loading
 *          timeouts and failures [Nick Benik] updated 11-9-09: Changes to
 *          announcement dialog functionality [Charles McGow] updated 11-23-09:
 *          Bug Fix for Firefox's 4k XML node text limit [Nick Benik]
 */
...
i2b2.PM._processUserConfigSuccess = function(data) {
    try {
        ...
        if (timeout == undefined || timeout < 300001) {
            i2b2.PM.model.IdleTimer.start(1800000 - 300000); // timeout);
            // //timeout-60000);
        } else {
            i2b2.PM.model.IdleTimer.start(timeout - 300000); // timeout);
            // //timeout-60000);
        }
    } catch (e) {
    ...
    // if user has more than one project display a modal dialog box to have them
    // select one
    var xml = data.refXML;
    ...
    for (var i = 0; i < projs.length; i++) {
         ...
         * //alert(roledetails[d].textContent); // BUG FIX - Firefox splits
         * large values into multiple 4k text nodes... use Firefox-specific
         * function to read concatenated value if ((roledetails[d].textContent) &&
         * (roledetails[d].textContent == "ADMIN")) { i2b2.PM.model.isAdmin =
         * true } else if ((roledetails[d].firstChild) &&
         * (roledetails[d].firstChild.nodeValue.unescapeHTML() == "ADMIN")) {
         * i2b2.PM.model.isAdmin = true } }
         */
        // details`
        ...
        for (var d = 0; d < projdetails.length; d++) {
            ...
            // BUG FIX - Firefox splits large values into multiple 4k text
            // nodes... use Firefox-specific function to read concatenated value
    ...
    if (!i2b2.PM.model.isAdmin && i2b2.PM.model.admin_only) {
        ...
        i2b2.PM.model.login_project = ""; // i2b2.h.XPath(projs[0],
        // 'attribute::id')[0].nodeValue;
        ...
        if (s.length > 0) {
            ...
            // } else if (s == null || s == "") {
            // alert("The PM Cell is down or the address in the properties file is
            // incorrect.");
        } else {
            alert("The PM Cell is down or the address in the properties file is incorrect.");
            // alert("Your account does not have access to any i2b2 projects.");
...
i2b2.PM.getEurekaClinicalSession = function(url, params) {
    var theIframe = new Element('iframe', {
        src: url + '/protected/get-session'
    });
    var timeout = null;
        ...
        try {
            ...
            if (newpass != retypepass) {
                ...
                scopedCallback.callback = function(results) {
                    // THIS function is used to process the AJAX results of the
                    // getChild call
                ...
                i2b2.PM.ajax.setPassword("Plugin:PM", {
                    sec_oldpassword: curpass,
                    sec_newpassword: newpass
                }, scopedCallback);
    ...
    showProjects: function() {
        ...
        if (!$("i2b2_projects_modal_dialog")) {
            ...
            if (!thisRef.yuiDialog) {
                ...
                var kl = new YAHOO.util.KeyListener("i2b2_projects_modal_dialog", {
                    keys: 13
                }, {
    ...
    showAnnouncement: function(msg) {
        ...
        if (!thisRef.yuiDialog) {
            ...
            thisRef.yuiDialog.cfg.queueProperty("buttons",
                [{
                        text: "Yes, I agree",
                        handler: i2b2.PM.view.modal.announcementDialog.clickOK,
                        isDefault: true
                    },
                    {
                        text: "No, I disagree",
                        handler: i2b2.PM.view.modal.announcementDialog.clickCancel
                    }
                ]);
            thisRef.yuiDialog.render(document.body);
            // show the form
...
// ================================================================================================================================
// NEW FRAMEWORK LAUNCH CODE (cells can timeout on failure instead of hanging
...
i2b2.PM._processLaunchFramework = function() {
    ...
    i2b2.events.afterCellInit.subscribe((function(type, args) {
        ...
        // all cells are loaded, fire the "all go" signal if all cells are
        // loaded
    ...
    for (var cellKey in i2b2.hive.cfg.lstCells) {
        if (cellIDs[cellKey]) {
            try {
                ...
                // do not save cell info unless the URL attribute has been set
                // (exception is PM cell)
            ...
            // if
            // (i2b2.PM.model.userRoles.indexOf(i2b2.hive.cfg.lstCells[cellKey].roles)
            // == -1) {
            // deleteKeys[cellKey] = true;
            // }
    ...
    /*
     * Legacy SHRINE code - Removed 6/3/16 // see if Shrine was loaded by the server
     * var t = i2b2.hive.cfg.lstCells["SHRINE"]; if (!Object.isUndefined(t) &&
     * t.serverLoaded) { i2b2.PM.model.shrine_domain = true; } delete t;
     */
